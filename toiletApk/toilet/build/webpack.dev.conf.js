...
module.exports = (env) => new Promise((resolve, reject) => {
    ...
    portfinder.getPortPromise().then((port) => {
        ...
        devWebpackConfig.plugins.unshift(new webpack.EnvironmentPlugin({
            NODE_ENV: 'development',
            DEBUG: env.debug || false // 不能为undefined
        }));
        if (env.debug) {
            devWebpackConfig.devServer.transportMode = {
              server: require.resolve('./debug-server'),
              client: 'ws'
            };
            devWebpackConfig.devServer.https = false;
        };
