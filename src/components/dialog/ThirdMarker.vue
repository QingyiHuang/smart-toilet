...
export default {
  ...
  methods: {
    ...
    deleteOne() {
      this.$emit('deleteMale', this.mdata.id);
    },
    getPosition() {
    ...
    getDirection(ev) {
      ...
      const yP = ev.offsetY;
      if (yP < offset) dir += 'n';
      else if (c.offsetHeight && yP > c.offsetHeight - offset) dir += 's';
      if (xP < offset) dir += 'w';
      else if (c.offsetWidth && xP > c.offsetWidth - offset) dir += 'e';
      return dir;
    },
    // 拖拽事件
    dragFunc(el) {
...
.maleMarker {
  ...
  height: 40px;
  position: absolute;
  background-color: rgba(31, 113, 221, 0.4);
  border: 1px solid rgb(103, 179, 250);
  &::before {
    content: '';
    width: 22px;
    height: 22px;
    ...
    margin-left: -11px;
    margin-top: -11px;
    background: url('~@/assets/guide/无障碍-空闲.png') no-repeat;
