...
const http = axios.create({
  ...
  baseURL: `${process.env.VUE_APP_API_PREFIX}/` // 接口基本路径
});
...
// 错误提示
function showError({ serviceErrorCode, code, msg, traceCode }) {
  msg = msg || i18n.t('common.text.systemError');
  // const alertError = code && code === -2;
  if (code !== '0') {
    // 没有错误码，消息提示
    // Message.error(msg);
    systemError({
...
http.interceptors.response.use(
  function (response) {
    ...
    // 对错误进行统一处理
    if (response.data.type !== -1 && response.data.type !== 0) {
      if (
