<template>
  ...
  >
    ...
    <div v-show="showWindow" class="window" @click.stop.prevent="() => {}">
      ...
      <div class="sendMesageBtn">短信通知</div>
      <!-- 关闭按钮 -->
...
export default {
  ...
  methods: {
    ...
    changePopShow() {
      // 如果当前display为block，那么不处理
      // console.log(this.$refs[`${this.popuData.id}`]);
      if (
...
.iconOuter {
  ...
  width: 36px;
  height: 36px;
  background: url('~@/assets/images/databoard/POI-公厕报警-默认1.png') no-repeat;
  ...
  .toiletName {
    ...
    left: -12px;
    top: -31px;
    border-left: 1px solid rgb(178, 74, 35);
    border-right: 1px solid rgb(178, 74, 35);
  }
  .window {
    height: 200px;
    width: 320px;
    background: url('~@/assets/images/databoard/tips-告警弹窗.png') no-repeat;
    ...
    .previewBtn {
      ...
      width: 80px;
      height: 32px;
      background: url('~@/assets/images/databoard/btn-黄按钮01-默认.png')
        no-repeat;
      background-size: 100% 100%;
    ...
    .closeBtn {
      background: url('~@/assets/images/databoard/btn-tips-关闭02-默认.png')
        no-repeat;
      background-size: 100% 100%;
      width: 56px;
      height: 26px;
...
.iconActive {
  background: url('~@/assets/images/databoard/POI-公厕报警-悬停.png') no-repeat;
  background-size: cover;
  width: 36px;
  height: 50px;
  left: -18px;
