    ...
    <div class="page full">
        <div class="page__hd">
            <div class="visitorHeader">
                <p>{{attenderData.name}} 您好</p>
                <p>{{renderData.inviterDept}}的{{renderData.inviterName}}现邀请您于{{renderData.startTime | timeFilter}}到{{renderData.meetingPlace}}参加{{renderData.meetingTheme}}。为来访当日顺利通行，请补充一下信息:</p>
            </div>
        ...
        <div class="page__bd">
            ...
            <!-- <div class="submit-btn">
                <div class="operate-btn text-center" @click="submit">确认提交</div>
            </div> -->
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            queryAttendee() {
                axios({
                    url:`/hvm-app/visitorNoCas/attendees/information`, 
                    data:{visitorId: this.visitorId}, 
                    headers: {'Content-Type': 'application/json;charset=UTF-8'},
                    method: 'post'
                }).then(res => {
                    if(res.data.code == 0){
                        this.renderData = res.data.data.invitation
                        this.attenderData = res.data.data.attender
                    }
                }).catch(err => {})
            },
            // 提交参会信息
            submitAttendee() {
                if(this.fileList.length <= 0 || this.faceUrl == '') {
                    ...
                    axios({
                        url: `/hvm-app/visitorNoCas/attendees/supplement`,
                        method: 'post',
                        headers: {'Content-Type': 'application/json;charset=UTF-8'},
                        data:{
                            "certNo": this.idCard,
